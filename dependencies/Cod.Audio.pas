...
interface
  ...
  type
    TPlayStatus = (Stopped, Playing, Stalled, Paused);
    TStreamOpenType = (StreamClosed, StreamFile, StreamURL);
...
begin
  ...
  FOpenedType := TStreamOpenType.StreamClosed;
end;
...
begin
  ...
  case FOpenedType of
    TStreamOpenType.StreamFile: Result.OpenFile(ChannelInfo.filename);
    TStreamOpenType.StreamURL: Result.OpenURL(ChannelInfo.filename);
  end;
end;
...
begin
  ...
  // Save Data
  FOpenedType := TStreamOpenType.StreamFile;
...
begin
  ...
  // Save Data
  FOpenedType := TStreamOpenType.StreamURL;
...
begin
  SetPlayStauts( TPlayStatus.Paused );
end;

procedure TAudioPlayer.Play;
begin
  SetPlayStauts( TPlayStatus.Playing );
end;
...
begin
  if GetFileOpened then
    begin
      ...
      case FOpenedType of
        TStreamOpenType.StreamFile: OpenFile(GetSampleInfo.filename);
        TStreamOpenType.StreamURL: OpenURL(GetSampleInfo.filename);
      end;
    end;
end;
...
begin
  case AStatus of
    TPlayStatus.Stopped: BASS_ChannelStop( FStream );
    TPlayStatus.Playing: BASS_ChannelPlay( FStream, false );
    TPlayStatus.Paused: BASS_ChannelPause( FStream );
  end;
end;

procedure TAudioPlayer.SetPosition(const Value: int64);
begin
  if (FOpenedType = TStreamOpenType.StreamURL) and FStreamDecoding then
    begin
...
begin
  ...
  SetPlayStauts( TPlayStatus.Stopped );
end;
