...
interface
  ...
  {$IFDEF MSWINDOWS}
    Windows, Graphics,
    {$IFNDEF FPC}Vcl.Imaging.jpeg, Vcl.Imaging.GIFImg, Vcl.Imaging.pngimage,
    Winapi.IpTypes, Winapi.IpHlpApi,{$ENDIF}URLMon, ActiveX, Variants, ComObj,
    WinInet
  {$ENDIF}
  {$IFNDEF FPC}, IOUtils{$ENDIF}, Cod.Files, Cod.StringUtils, Cod.MesssageConst
...
type
  ...
  THTTPDownloadWorkProc = {$IFNDEF FPC}reference to{$ENDIF} procedure(var Cancel: boolean; Work, WorkTotal: int64);
...
begin
  ...
  try
    ...
    FileStream := TFileStream.Create(Destination, Classes.fmCreate);
    try
  ...
  try
    try
      ...
      if downloadfallback then begin
        FilePath := 'C:\Windows\Temp\tempinternetstream' + GenerateString(10, [TStrGenFlag.LowercaseLetters, TStrGenFlag.Numbers]);
...
begin
  ...
  for X := Low(TNetworkProtocol) to High(TNetworkProtocol) do
