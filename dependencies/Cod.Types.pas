...
interface
uses
  SysUtils, Classes, Variants, Math, Types, DateUtils, Generics.Collections,
  Generics.Defaults{$IFDEF FPC}, Cod.Platform.Lazarus{$ENDIF};
...
type
  ...
  public
    ...
    class procedure Switch(var Left, Right: T); static;
    class function Compare(const Left, Right: T): TValueRelationship; static;
...
var
  ...
  ColCount, RowCount, DivTotal: integer;
  ElemSize: TSize;
...
begin
  ...
  case Layout.ContentFill of
    ...
    TRectLayoutContentFill.Fill: begin
      ...
      // Get proportions
      Scale := Element.Height * (Base.Width / Element.Width);
...
begin
  for I := 0 to High(Cases) do
    for J := 0 to High(Cases[I].Values) do

      if {$IFNDEF FPC}TComparer<T>.Default.Compare(Cases[I].Values[J], Value) = EqualsValue
      {$ELSE}Cases[I].Values[J] = Value{$ENDIF} then begin
        Cases[I].Execute;
        Exit;
      end;
...
class function TType<T>.Compare(const Left, Right: T): TValueRelationship;
begin
...
class procedure TType<T>.Switch(var Left, Right: T);
var
  Temp: T;
begin
  Temp := Left;
