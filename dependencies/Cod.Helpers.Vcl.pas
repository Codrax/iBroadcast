...
interface
uses
  SysUtils, Classes, IdHTTP, IniFiles,
  {$IFDEF MSWINDOWS}
  Windows, {$IFNDEF FPC}WinApi.GdipObj, WinApi.GdipApi, Cod.GDI, {$ENDIF}
  {$ENDIF}
  {$IFNDEF FPC}Vcl.Themes,{$ENDIF}
  Graphics, ActiveX, Generics.Collections, Generics.Defaults,
  Dialogs, Cod.Helpers, Cod.Types, DateUtils, Menus, Types,
...
begin
  ...
  try
    SetString(Result, Buffer, GetPrivateProfileString(PChar(Section),
      PChar(Ident), PChar(Default), Buffer, Length(Buffer),
      PChar(FileName)));
  finally
