...
interface
uses
  Windows, Messages, Classes, Graphics, Types, Math, SysUtils,
  {$IFNDEF FPC}Vcl.Imaging.pngimage, Vcl.Imaging.GIFImg, Vcl.Imaging.jpeg,
  Cod.GDI, {$ENDIF}
  Cod.Helpers, Cod.Helpers.Vcl, Cod.Types, Cod.StringUtils,
  Cod.ArrayHelpers, Contnrs, Forms
...
procedure DrawImageInRect(Canvas: TCanvas; Rect: TRect; Image: TGraphic;
  Layout: TRectLayout; ClipImage: boolean = false{$IFNDEF FPC}; Opacity: byte = 255{$ENDIF}); overload;
procedure DrawImageInRect(Canvas: TCanvas; Rect: TRect; Image: TGraphic;
  DrawMode: TDrawMode = TDrawMode.Fill; ImageMargin: integer = 0;
  ClipImage: boolean = false{$IFNDEF FPC}; Opacity: byte = 255{$ENDIF}); overload;
...
begin
  ...
  Style.Clipping:=not (TTextFlag.NoClip in Flags);
  Style.EndEllipsis:=TTextFlag.Ellipsis in Flags;
  Style.ShowPrefix:=TTextFlag.ShowAccelChar in Flags;
  {$ELSE}
  TextFormat := [];
  ...
  if TTextFlag.WordWrap in Flags then
    begin
      ...
      for I := 0 to High(Lines) do
        begin
          ...
          Canvas.TextRect(R,
            {$IFDEF FPC}ARect.Left, ARect.Top, {$ENDIF}
            Lines[I], {$IFDEF FPC}Style{$ELSE}TextFormat{$ENDIF});
    ...
    begin
      ...
      if TTextFlag.Top in Flags then
         Style.Layout := tlTop;
      if TTextFlag.VerticalCenter in Flags then
      ...
      Canvas.TextRect(ARect,
        {$IFDEF FPC}ARect.Left, ARect.Top, {$ENDIF}
        Text, {$IFDEF FPC}Style{$ELSE}TextFormat{$ENDIF});
    end;
end;
...
begin
  ...
  Destination.Canvas.CopyRect(
    ...
    Source.Canvas,
    Rect(MinX, MinY, MaxX + 1, MaxY + 1){$IFNDEF FPC},
    255
    {$ENDIF}
  );
...
procedure DrawImageInRect(Canvas: TCanvas; Rect: TRect; Image: TGraphic;
  DrawMode: TDrawMode; ImageMargin: integer; ClipImage: boolean{$IFNDEF FPC}; Opacity: byte{$ENDIF});
var
  Layout: TRectLayout;
begin
  ...
  // Data
  DrawImageInRect(Canvas, Rect, Image, Layout, ClipImage{$IFNDEF FPC}, Opacity{$ENDIF});
end;

procedure DrawImageInRect(Canvas: TCanvas; Rect: TRect; Image: TGraphic;
  Layout: TRectLayout; ClipImage: boolean = false{$IFNDEF FPC}; Opacity: byte = 255{$ENDIF});
const
  PIXEL_BYTE_SIZE = 4;
var
  Rects: TArray<TRect>;
  ...
  if not ClipImage then
    ...
    begin
      for I := 0 to High( Rects ) do
        Canvas.StretchDraw( Rects[I], Image{$IFNDEF FPC}, Opacity{$ENDIF});
    end
  else
    // Clip Image Drw
    begin
      for I := 0 to High(Rects) do
        begin
          ...
          try
            ...
            Bitmap.Canvas.StretchDraw(FRect, Image{$IFNDEF FPC}, 255{$ENDIF}); // Full opacity for temp

            // Image has no alpha channel, set A bytes to 255
            if not Image.Transparent then begin
              RectZone := TRect.Intersect(Bitmap.Canvas.ClipRect, FRect);

              for Y := RectZone.Top to RectZone.Bottom-1 do begin
                // Line
                Pos:= Bitmap.ScanLine[Y];

                // Start left
                for X := RectZone.Left to RectZone.Right-1 do
            ...
            Canvas.Draw(Rect.Left, Rect.Top, BitMap{$IFNDEF FPC}, Opacity{$ENDIF});
          finally
...
begin
  ...
  // Draw
  for I := -(FlowerSize-1) to FlowerSize-1 do begin
    ...
    // Vertical
    for J := -(FlowerSize-1)+Abs(I div 2) to FlowerSize-1-Abs(I div 2) do begin
      Y := Center.Y + Add.cy * J;
...
begin
  Handle := FindWindow(nil, PChar(ApplicationCaption));
...
{$IFNDEF FPC}
{$IFDEF MSWINDOWS}
function ColorToTriple(Color: TColor): Windows.TRGBTriple;
var
  ColorRGB: Longint;
begin
...
begin
  ...
  try
    ...
    {$ELSE}
    Bitmap.Canvas.StretchDraw(Rect(0, 0, Bitmap.Width, Bitmap.Height), Mipmap);
    {$ENDIF}
  finally
    Mipmap.Free;
  end;
