...
uses
  {$IFDEF MSWINDOWS}
  Windows, ShellAPI, Registry, ComObj,
  {$ENDIF}
  SysUtils, Variants, Classes, Math, Cod.ArrayHelpers
...
begin
  ...
  for x := 1 to Length(Result) do
    if CharInSet(Result[x], DisallowedCharSet) then
      Result[x] := '_';
end;
...
begin
  for x := 1 to Length(AString) do
    if CharInSet(AString[x], DisallowedCharSet) then
      Exit(false);
...
begin
  ...
  if not GetUserName(PChar(Result), dwSize) then
    RaiseLastOSError;
end;
